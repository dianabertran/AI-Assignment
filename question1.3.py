...
def generalSolution(distance):

    """
    This function is used for users to input their individual
    personalised design of grid positions and arrangements.

    Users can choose their tile positions accordingly

    However, they must follow a set of rules:
    --> Numbers have to be between 0-8
    --> Numbers cannot be repeated
    """

    print("\nHello and welcome to the 8-puzzle problem general solution!"
          "\nPlease choose the numbers for the  " + distance +
          " Position of the tiles. \nUtilise 0 to represent the empty tile on the grid."
          "\nPlease input any numbers ranging from 0-8."
          "\nEnjoy! ")
    gridArrangement = np.zeros(9, dtype=int)
    showGrid(gridArrangement)
    for value in np.nditer(gridArrangement, op_flags=['readwrite']):
        correctValue = False
        userInput = int(input("INPUT HERE"))
        while not correctValue:
            if (userInput not in gridArrangement or userInput == 0) and (userInput < 9 and userInput > -1):
                correctValue = True
            else:
                print("ERROR! Please follow the rules:"
                      "--> Integers must be from 0-8"
                      "--> Integers must only be used once.")
                showGrid(gridArrangement)
                userInput = int(input(distance + " number: "))
        value[...] = userInput
        showGrid(gridArrangement)
...
def main():
    ...
    while distance != "1" and distance != "2":
        distance = input(
            "Input either 1 or 2 to solve the puzzle using a distinct function:\n 1. Manhattan Distance 2. Euclidean Distance")
    # states initial position
    initialPosition = generalSolution("Initial")
    # states final position
    finalPosition = generalSolution("Final")
